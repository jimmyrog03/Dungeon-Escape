...
segment code ;Code Segment
	...
	terminate:
		...
		; Stop processes related to sound
	...
	secDelay:			
		; Roughly delays for a second when called
		push AX
		MOV AX,0FFFFh
		call restLoop
		call restLoop
...
;start of char
    draw_character:
		...
		je movingLeft			; If character needs to be drawn moving left
...
;Page 7 => Loading Screen
	setDeathScreen:
		; Sets displayed page to page 2
		push ax
		Mov AH,05h
		MOV AL,02 
		INT 10h	; INT 10h, AH = 05h, AL = page # | Set active display page
		pop ax
	ret
	deathScreen:
		push bp
    ...
    ret
    setCreditScreen:
		; Sets displayed screen to page 6
		push ax
		Mov AH,05h
		MOV AL,06	
		INT 10h	; INT 10h, AH = 05h, AL = page # | Set active display page
    ...
    creditInput:
		; Waits for any input and then returns
        MOV AH,00h
		INT 16h  ; INT 16h, AH = 00h | Keyboard Status
        mov word [currentScreen], 3
    ret
    ...
    ;Drawing the game Screen is too complicated to include here
	
    gameInput:
		; Evaluates user input during game sequence
        ;	CURRENT SUPPORTED INPUTS
		;	Q=>quit | W=>up | S=>down | A=>left | D=>right | Space=>toggle walk distance
        call draw_character
...
; Wait Screen that is set between rooms being drawn so the process is not seen by the player
    ...
    ret
     pausedInput:
        ...
        MOV AH,00h
        INT 16h   
        CMP AH, 13h	;Player inputted r
        je resumeGame
        CMP AH, 1fh	;Player inputted s
        je saveGame
        cmp AH, 10h	;Player inputted q
        je quitYesSave
		CMP AH, 32h ;Player inputted m
		je quitNoSave
